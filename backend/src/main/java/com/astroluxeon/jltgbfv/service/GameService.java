...
public class GameService {
    ...
    public List<Challenge> getChallenges() {
        return gameData.getChallenges();
    }

    public List<Challenge> getBattles() {
        return gameData.getBattles();
    }

    public Collection<Region> getRegions() {
        return gameData.getRegions().values();
    }
    ...
    public synchronized void completeChallenge(int id, String team) {
        ...
        for (Challenge c : gameData.getChallenges()) {
            if (c.getId() == id) {
                challenge = c;
                break;
            }
        ...
        if (challenge != null && challenge.getStatus() == Status.ACTIVE) {
            ...
            Challenge c = drawCard(gameData.getChallenges());
            int id2 = c != null ? c.getId() : -1;
            gameData.getChallengeHistory().push(new Pair<>(id, id2));
            saveToFile();
        }
    }

    public Challenge startBattle() {
        Challenge c = drawCard(gameData.getBattles());
        saveToFile();
        return c;
    }
    ...
    public synchronized void completeBattle(int id, String team) {
        ...
        for (Challenge c : gameData.getBattles()) {
            if (c.getId() == id) {
                battle = c;
                break;
            }
    ...
    public void claimRegion(String id, String team) {
        if (gameData.getRegions().get(id) != null) {
            try {
                gameData.getRegions().get(id).setTeam(Team.valueOf(team));
                saveToFile();
                System.out.println(gameData.getRegions().get(id).getName() + " claimed by " + team);
            } catch (Exception e) {
    ...
    public void lockRegion(String id, boolean lock) {
        if (gameData.getRegions().get(id) != null) {
            gameData.getRegions().get(id).setLocked(lock);
            saveToFile();
            if (lock) {
                System.out.println(gameData.getRegions().get(id).getName() + " locked by " + gameData.getRegions().get(id).getTeam());
            }
        }
    }

    public void undoChallenge() {
        if (gameData.getChallengeHistory().isEmpty()) {
            return;
        }

        Pair<Integer, Integer> top = gameData.getChallengeHistory().pop();
        int id1 = top.key();
        int id2 = top.value();

        for (Challenge challenge : gameData.getChallenges()) {
    ...
    public void initializeNewGame() {
        ...
        if (file.exists()) {
            try {
                this.gameData = objectMapper.readValue(file, GameData.class);
            } catch (IOException e) {
                System.out.println("Error initializing game: " + e.getMessage());
                this.gameData = new GameData(new HashMap<>(), new ArrayList<>(), new ArrayList<>(), new Stack<>());
            }
        } else {
        ...
        for (int i = 0; i < ACTIVE_DECK_SIZE; i++) {
            drawCard(this.gameData.getChallenges());
        }
    ...
    private void loadFromFile() {
        ...
        if (file.exists()) {
            try {
                this.gameData = objectMapper.readValue(file, GameData.class);
                if (this.gameData.getChallengeHistory() == null) {
                    this.gameData.setChallengeHistory(new Stack<>());
                }
    ...
    private void saveToFile() {
        try {
            objectMapper.writeValue(new File(DATA_FILE), this.gameData);
        } catch (IOException e) {
